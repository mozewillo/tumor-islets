...
def main():
    ...
    if args.testing == 'local':
        only_files = [my_dir + "IMMU-NSCLC-0179-FIXT-01-IF1-01.tsv"]
    elif args.testing == 'memory':
        only_files = [my_dir + "IMMU-NSCLC-0182-FIXT-01-IF1-01.tsv"]
    else:
        only_files = [my_dir + f for f in listdir(my_dir) if isfile(join(my_dir, f)) if "tsv" in f]
        if args.max_panels is not None and args.max_panels < len(only_files):
            only_files = only_files[:args.max_panels]

    for filename in only_files:
        inv_marg_file = filename.replace("tsv-files", "invasive_margins_" + today.strftime("%y_%m_%d")).replace(".tsv", "")
